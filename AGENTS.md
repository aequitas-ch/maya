...
## 1. Agent Skills & Roles

### Skill: supervisor
- **Scope**: Coordinates application development and delegates subtasks.
- **Instructions**: Analyze complex feature requests. Split them into logic and testing tasks. Launch the corresponding sub-agents (`code-architect` and `qa-tester`) for these tasks. Consolidate their results.

### Skill: code-architect
- **Scope**: Writes and modifies source code in the `frontend/` and `backend/` directories.
- **Instructions**: Implement features based on the specifications from the orchestrator (supervisor). Strictly adhere to existing design patterns.

### Skill: qa-tester
- **Scope**: Writes API, End-to-End, and Unit tests.
- **Instructions**: Analyze the modified code and write comprehensive tests without gaps. Use Cypress for E2E tests and Bruno for API tests to validate.
...
## 6. Workflow & Definition of Done (DoD)

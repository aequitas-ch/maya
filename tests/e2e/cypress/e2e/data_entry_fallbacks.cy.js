describe('Data Entry with Fallbacks', () => {
  ...
  it('preserves form data and shows an error message when submission fails due to network drop', () => {
    ...
    cy.get('input[placeholder="e.g. kg, cm"]').parent().next().find('input').type('120');
    ...
    // Look for generic error boundary or the specific form error
    ...
    cy.get('input[placeholder="e.g. Weight, Height"]').should('have.value', 'Height');
    cy.get('input[placeholder="e.g. kg, cm"]').should('have.value', 'cm');
    cy.get('input[placeholder="e.g. kg, cm"]').parent().next().find('input').should('have.value', '120');
